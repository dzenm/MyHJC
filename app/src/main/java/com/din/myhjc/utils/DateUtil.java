...
public class DateUtil {
    ...
    private Date dates, dateSimple;
    private Calendar calendar, calendarDate, calendarTime;
    ...
    public String DateToWeek(String weekDate) {
        ...
        try {
            ...
            //-------- 将时间设置为固定格式并赋值给date
            dates = simpleDateFormat.parse(weekDate);
    ...
    public String DateToSimple() {
        ...
        date = simpleDateFormat.format(dateSimple);
        //-------- calendar的实例设置时间为date
        return date;
    }
     ...
     * @param context
     */
    public void dateDialog(final Context context, final TextView date) {
        calendarDate = Calendar.getInstance();
        new DatePickerDialog(context, new DatePickerDialog.OnDateSetListener() {
            @Override
            public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
